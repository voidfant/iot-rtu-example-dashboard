    ...
    if (active && payload && payload.length) {
      ...
      <div className="rounded-xl overflow-hidden tooltip-head">
        <div className="flex items-center justify-between p-2">
          <div className="">{`Package №${payload[0].payload['id']}`}</div>
          {console.log(payload)}
          <Icon path="res-react-dash-options" className="w-2 h-2" />
        </div>
        {payload.map((pld) => (
          <div className="tooltip-body text-center p-3">
            <div className="text-white font-bold">{pld.value}</div>
            <div className="">{pld.dataKey}</div>
          </div>
        ))}
      </div>
      )
    ...
    <div className="flex p-4 h-full flex-col">
      ...
      <div className="">
        <div className="flex items-center">
          ...
          }}>{!isPaused ? 'Close connection' : 'Open connection'}</button>
          <div className="ml-6 w-5 h-5 flex justify-center items-center rounded-full icon-background">
            ?
          </div>
        </div>
        <div className="font-bold ml-5">Meters</div>
      </div>
      
      {!!ws_data &&
      <div className="flex-grow">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart width={500} height={300} data={gData}>
            <defs>
              <linearGradient id="paint0_linear" x1="0" y1="0" x2="1" y2="0">
